...
const updateProfile=asyncHandler(async (req,res,next)=>{
   ...
   if(req.body.cover){
      if(!JSON.stringify(user.cover) === '{}'){
      await cloudinary.v2.uploader.destroy(user.cover.public_Id);
      };
      const cloud= await cloudinary.v2.uploader.upload(req.body.cover,{
         folder: "Post",
      });
      user.cover.public_Id=cloud.public_id;
      user.cover.public_Url=cloud.secure_url;
   };
   if(req.body.firstName){
      console.log(req.body.firstName);
      user.about.firstName=req.body.firstName;
   };
   if(req.body.lastName){
      user.about.lastName = req.body.lastName;
   };
   if(req.body.country){
      user.about.country = req.body.country;
   };
   if(req.body.relationShipStatus){
      user.about.relation = req.body.relationShipStatus;
   };
   if(req.body.workAt){
      user.about.workAt = req.body.workAt;
   };
   if(req.body.livesIn){
      user.about.livesIn = req.body.livesIn;
   };
   await user.save();
   res.status(200).json({
      success:true,
